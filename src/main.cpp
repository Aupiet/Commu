...
void setup() {
  ...
  // NOTE: packetQueue a été supprimé car on traite le LiDAR en direct
  // maintenant
  ...
  xTaskCreatePinnedToCore(microRosLidarTask, "microROS_Lidar", 24000, NULL, 5,
                          NULL, 1);

  // Priorité 3 : Moteurs
  xTaskCreatePinnedToCore(motorControlTask, "Motors", 4096, NULL, 3, NULL, 1);

  // Priorité 3 : Navigation naïve
