                ...
                IMU_ST_SENSOR_DATA *pstMagnRawData)
{
  ...
  MotionVal[0]=gyro[0]; MotionVal[1]=gyro[1]; MotionVal[2]=gyro[2];
  MotionVal[3]=acc[0];  MotionVal[4]=acc[1];  MotionVal[5]=acc[2];
  ...
  pstAngles->roll = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3; 
...
void calibrateMagn(void)
{
  ...
  delay(2000);
  magnetometer_.getData(&x, &y, &z);
  temp[3] = x; temp[4] = y; temp[5] = z;

  Serial.printf("flip device\n");
  delay(2000);
  magnetometer_.getData(&x, &y, &z);
  temp[6] = x; temp[7] = y; temp[8] = z;
