...
const uint16_t MIN_PWM = 60; // Augmenté légèrement pour éviter le grognement moteur
...
void stopMotors() {
  // Freinage actif (Low/Low coupe l'alimentation, mais ne freine pas fort. High/High freinerait)
  digitalWrite(AIN1, LOW); digitalWrite(AIN2, LOW); ledcWrite(MOTOR_CHANNEL_A, 0);
  digitalWrite(BIN1, LOW); digitalWrite(BIN2, LOW); ledcWrite(MOTOR_CHANNEL_B, 0);
}
...
void motorControlTask(void *pvParameters) {
  ...
  while (true) {
    ...
    // Récupération atomique des commandes
    ...
    // --- SÉCURITÉ LIDAR ---
    bool isMovingForward = (joyY > 50); // Tolérance de 50 sur 512
